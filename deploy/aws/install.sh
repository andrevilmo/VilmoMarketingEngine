...
cd /opt/vilmo
COMPOSE=(docker compose -f docker-compose.yml -f aws/docker-compose.aws.yml)
if [[ -f /etc/letsencrypt/live/vilmomkt.com/fullchain.pem ]]; then
  COMPOSE+=(-f aws/docker-compose.tls.yml)
fi
"${COMPOSE[@]}" up -d --build
