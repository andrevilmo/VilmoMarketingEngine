    ...
    IHttpClientFactory httpFactory)
{
    ...
    public async Task PublishAsync(Advertisement ad, Listing listing, Guid runId, CancellationToken ct)
    {
        ...
        if (IsMissingMercadoLivrePictures(ad, listing, out var rawPictures))
        {
            listing.Status = ListingStatuses.Error;
            listing.RemoteStatus = "error";
            listing.LastSyncedAt = DateTimeOffset.UtcNow;
            await logs.WriteAsync(ad.CompanyId, runId, ad.Id, listing.Id, listing.MarketplaceCode, action,
                ListingPublishLogSteps.Failed, "error",
                "O Mercado Livre exige ao menos uma foto pública (HTTPS) no campo Imagens ML. Sem foto o anúncio Clássica é recusado (item.listing_type_id.requiresPictures).",
                new { pictures = rawPictures }, ct);
            return;
        }
    ...
    static (string Method, string Url, object? Payload) BuildRequest(Advertisement ad, Listing listing, string baseUrl, string action)
    {
        ...
        if (listing.MarketplaceCode.Equals("MercadoLivre", StringComparison.OrdinalIgnoreCase))
        {
            ...
            var pictures = MercadoLivrePictures.Parse(Attr(attrs, MercadoLivrePictures.FieldName));
            if (pictures.Count > 0)
                payload["pictures"] = MercadoLivrePictures.ToPayload(pictures);
        }
    ...
    static bool IsMissingMercadoLivrePictures(Advertisement ad, Listing listing, out string? raw)
    {
        raw = ad.Attributes
            .FirstOrDefault(a =>
                a.MarketplaceCode.Equals(listing.MarketplaceCode, StringComparison.OrdinalIgnoreCase)
                && a.FieldName.Equals(MercadoLivrePictures.FieldName, StringComparison.OrdinalIgnoreCase))
            ?.FieldValue;
        if (!listing.MarketplaceCode.Equals("MercadoLivre", StringComparison.OrdinalIgnoreCase))
            return false;
        return MercadoLivrePictures.Parse(raw).Count == 0;
    }
