...
| Advertisements | `#/anuncios` form: **Um produto** or **Conjunto / kit**; marketplace checkboxes; common fields; per-channel extras; item rows (SKU + qty in the set). Save as **rascunho**. Each saved ad shows per-channel **Cancelar** / **Publicar neste canal** plus **Atualizar dados online**. | Save for the **selected company**. Kit: N SKUs, each qty ≥ 1. Paid sale explodes kit BOM into stock. Vendor: own ads. |
